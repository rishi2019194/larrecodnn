...
namespace keras {
  ...
  void conv_single_depth_valid(std::vector<std::vector<float>>& y,
                               std::vector<std::vector<float>> const& im,
                               std::vector<std::vector<float>> const& k);
  void conv_single_depth_same(std::vector<std::vector<float>>& y,
                              std::vector<std::vector<float>> const& im,
                              std::vector<std::vector<float>> const& k);
...
public:
  ...
  void show_name()
  {
    std::cout << "DataChunk2D " << data.size() << "x" << data[0].size() << "x" << data[0][0].size()
              << std::endl;
  }
...
public:
  ...
  void show_name() { std::cout << "DataChunkFlat " << f.size() << std::endl; }
  void show_values()
  {
